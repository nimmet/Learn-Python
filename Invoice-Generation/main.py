...
for filepath in filepaths:
    ...
    pdf.cell(w=30, h=8, txt="", border=1)
    pdf.cell(w=30, h=8, txt=str(f"{total:.0f}"), border=1, ln=1)

    pdf.set_font(family="Times", style="B", size=12)
    # pdf.text(txt=f"The total due amount is {total:.0f} Euros.", x=10, y=75)
    pdf.cell(w=25, h=8, txt=f"The total due amount is {total:.0f} Euros.", ln=1)
